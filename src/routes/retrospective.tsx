...
function Retrospective() {
			...
			<div className="min-h-screen bg-background p-6">
				<div className="max-w-4xl mx-auto">
					...
					<div className="bg-card rounded-lg shadow-lg p-8 border border-border text-center">
						...
						<h2 className="text-xl font-semibold mb-2 text-card-foreground">{t('retrospective.noData')}</h2>
						<p className="text-muted-foreground mb-6">{t('retrospective.noDataDescription')}</p>
						<Link
							to="/"
	...
	const completedItems = selectedMeeting?.agendaItems.filter(item => item.actualMinutes) || []
	const totalEstimated = completedItems.reduce((sum, item) => sum + item.estimatedMinutes, 0)
	const totalActual = completedItems.reduce((sum, item) => sum + (item.actualMinutes || 0), 0)
				...
				<div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
					...
					<div className="lg:col-span-1">
						<div className="bg-card rounded-lg shadow-lg p-6 border border-border">
							<h2 className="text-lg font-semibold mb-4 text-card-foreground">{t('history.title')}</h2>
								...
								{meetingHistory.map((meeting) => (
									...
									>
										...
										<div className="text-xs opacity-75">
											{meeting.agendaItems.length} {t('history.items')} • {formatTime(meeting.agendaItems.reduce((sum, item) => sum + (item.actualMinutes || 0), 0))}
										</div>
									</button>
								))}
							</div>
							...
							<>
								...
								<div className="bg-card rounded-lg shadow-lg p-6 border border-border">
									<h2 className="text-xl font-semibold mb-4 text-card-foreground">
										{t('retrospective.timeAnalysis')} - {new Date(selectedMeeting.date).toLocaleDateString()}
									</h2>
